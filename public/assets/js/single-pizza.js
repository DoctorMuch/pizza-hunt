...
function getPizza() {
  ...
  fetch(`api/pizzas/${pizzaId}`)
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a pizza by this id! Taking you back.');
      window.history.back();
    });
}
...
function handleNewCommentSubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}`, {
    ...
    .then(response => {
      if(!response.ok) {
        throw new Error({ message: 'Something went wrong! Apologies.' });
      }
      response.json();
    })
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${pizzaId}/${commentId}`, {
    ...
    .then(response => {
      if (!response.ok) {
        throw new Error({ message: 'Something strange has happened.' });
      }
      response.json();
    })
    .then(replyResponse => {
